...
func (s *emailService) VerifyEmail(ctx context.Context, token string) error {
	...
	// 更新用户邮箱验证状态
	user, err := s.userRepo.GetByID(ctx, verificationToken.TenantID, verificationToken.UserID)
	if err != nil {
	...
	// 标记令牌为已使用
	if err := s.verificationRepo.MarkAsUsed(ctx, verificationToken.ID); err != nil {
