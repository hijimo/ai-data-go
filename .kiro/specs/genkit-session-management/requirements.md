...
- **向量检索（Vector Search）**: 基于语义相似度的记忆检索，使用 Qdrant 向量数据库
- **Qdrant**: 高性能向量数据库，用于存储和检索向量嵌入
- **Collection**: Qdrant 中的向量集合，本系统使用单个共享 Collection 存储所有租户的向量
- **Payload**: Qdrant 向量的元数据，包含租户 ID、会话 ID 等过滤字段
...
1. THE 应用 SHALL 创建 conversation_memories 表存储长期记忆的元数据
2. THE 应用 SHALL 创建 conversation_contexts 表存储上下文配置
3. THE 应用 SHALL 使用 Qdrant 向量数据库存储向量嵌入
4. THE 应用 SHALL 为所有表的主键使用 UUID 类型
5. THE 应用 SHALL 为所有表设置主键默认值为 gen_random_uuid()
6. THE 应用 SHALL 在 Qdrant 中使用单个共享 Collection（conversation_memories）存储所有租户的向量
7. THE 应用 SHALL 在 Qdrant 的 Payload 中包含 tenant_id 字段并设置 is_tenant=true 索引
8. THE 应用 SHALL 在 Qdrant 的 Payload 中包含 session_id、memory_type 等元数据字段
9. THE 应用 SHALL 配置 Qdrant 的自定义分片策略以按租户分布数据
10. THE 应用 SHALL 在所有表中包含 tenant_id 字段以支持多租户隔离
11. THE 应用 SHALL 在所有表中包含 is_deleted 字段以支持软删除
12. THE 应用 SHALL 在所有向量检索操作中强制按 tenant_id 过滤
...
2. THE memorySearchFlow SHALL 使用 Qdrant 执行相似度搜索
3. THE memorySearchFlow SHALL 过滤相似度低于最小阈值的结果
4. THE memorySearchFlow SHALL 使用 Qdrant 的 filter 功能应用时间范围过滤（如果指定）
5. THE memorySearchFlow SHALL 使用 Qdrant 的 filter 功能应用记忆类型过滤（如果指定）
6. THE memorySearchFlow SHALL 使用 Qdrant 的 payload 功能计算综合分数（相似度 × 重要性）
...
12. WHERE IncludeCrossSessions 为 true 时，THE memorySearchFlow SHALL 使用 Qdrant filter 仅检索同一租户内的记忆
...
6. THE memoryStoreFlow SHALL 保存记忆元数据到 PostgreSQL 数据库
7. THE memoryStoreFlow SHALL 保存向量到 Qdrant 向量数据库
8. WHERE 指定过期时间时，THE memoryStoreFlow SHALL 在 Qdrant payload 中设置过期时间
9. THE memoryStoreFlow SHALL 在 Qdrant payload 中存储租户 ID、会话 ID 和记忆 ID
10. THE memoryStoreFlow SHALL 建立记忆与消息的关联关系
11. THE memoryStoreFlow SHALL 在 500 毫秒内完成存储
12. WHEN 向量生成失败时，THE memoryStoreFlow SHALL 重试 2 次
...
2. THE 系统 SHALL 在 Qdrant 向量检索中使用 filter 限制搜索范围在当前租户内
3. THE 系统 SHALL 在缓存键中包含租户 ID
4. THE 系统 SHALL 使用参数化查询防止 SQL 注入
5. THE 系统 SHALL 在 tenant_id 字段上创建索引以优化查询性能
6. THE 系统 SHALL 在 Qdrant payload 中包含 tenant_id 字段并创建索引
...
6. THE 系统 SHALL 检查 Qdrant 向量数据库可用性
...
3. WHEN Qdrant 向量数据库故障时，THE 系统 SHALL 使用全文搜索作为降级方案
4. WHEN Qdrant 和全文搜索都失败时，THE 系统 SHALL 跳过长期记忆检索
